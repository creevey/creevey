...
export function withCreeveyTests(
  ...
  return class extends React.Component<CreeveyTestsProviderProps, CreeveyTestsProviderState> {
    ...
    handleStartStoryTests = (): void => {
      const ids: string[] = Object.values(this.state.status.tests)
        .filter((x) => x?.storyId === this.state.storyId)
        .map((x) => x?.id ?? '')
        .filter(Boolean);
      ...
      return this.props.active ? (
        <CreeveyContext.Provider
          value={{
            ...
            onStartStoryTests: this.handleStartStoryTests,
